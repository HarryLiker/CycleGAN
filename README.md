...
Задача выполнить задачу по преобразованию изображений с помощь моделей CycleGAN. 
...
Конволюционный блок состоит из сверточного слоя, который выполняет свертку изображения с помощью ядер свертки, слоя InstanceNorm2d, применяемый для нормализации экземпляра входного тензора, а также функции активации. Для созданного блока можно изменять параметры каналов, размеры ядра, сдвиг по тензору, паддинг, также есть вощможность отражения границ изображения для сохранения информации о краях изображения. Также есть возможность убирать функцию активации, а также изменять её.
...
Блок остаточных связей состоит из двух конволюционных блоков. В отличие от сверточных блоков он сохраняет информацию из предыдущего слоя и добавляет его к итоговому выходу. 
...
При подаче тензора изображения генератору в начале выполняется увеличение количества каналов с сохранением размера тензора. Далее выполняется уменьшение размерности (Downsampling), пропускание через блоки остаточных связей, после чего выполняется увеличение размерности (Upsampling) и возвращение тензора с размерности, в которой он был подан генератору в начале.
...
![cycle_loss](report_imgs/discriminator_onnx.png)
...
Функция **Cycle loss** находит ошибку между изначальным изображением и изображением, которое было преобразовано сначала в другой тип, а потом преобразовано обратно в изначальный тип (например A->B->A). В таком случае для генератора будет хорошо, если изображение, которое было пройдено через цикл, практически не будет отличаться от того, каким оно было изначально до прохождения по циклу.
...
Функция **Identity loss** находит ошибку между изображением, которое уже должно быть преобразовано и изображением, которое получается при его пропускании через генератор (например $G(Y) \rightarrow Y$, где G - функция преобразования изображений $X$ в изображения $Y$). В таком случае для генератора будет хорошо, если изображение, поданное генератору, которое уже не нужно изменять, после прохода через генератор останется идентичным, потому что оно соответствует уже итоговому, в которое оно должно быть преобразовано.
...
Общая ошибка для обоих генераторов. Значение $\lambda$ показывает насколько весомы ошибки по циклу ($L_{Cycle}$) и по идентичности ($L_{Identity}$) при обучении модели **CycleGAN**.
...
Функцию обучения модели на одной эпохе можно посмотреть [здесь](train.py#L136).

<a name="train_horse2zebra"><h3>3.1. Обучение на датасете Horse2Zebra (Задача 1)</h3></a> 

Модель CycleGAN была обучена на датасете [horse2zebra](https://people.eecs.berkeley.edu/~taesung_park/CycleGAN/datasets/horse2zebra.zip).
...
<a name="train_forest_fire"><h3>2.2. Обучение на датасете Forest Fire (Задача 2)</h3></a> 
Модель CycleGAN была обучена на датасете [Forest Fire](https://www.kaggle.com/datasets/alik05/forest-fire-dataset).

После установки датасета изображения были распределены по папкам для верного разделения изображений при обучении.
...
```
python3 evaluate.py --weights_path ./weights/forest_fire/ --imgs_dir new_forest --results_path forest_eval_results
```
...
При запуске обучения модели с нуля все остальные необходимые файлы будут созданы автоматически:
* train_history - директория, где хранится история обучения на различных датасетах. Хранит значения функций ошибок, изображение со всеми ошибками на всем обучении, а также результаты моделей (изображения) на части тестовой выборки на определенных эпохах.
* weights - директория, где хранятся веса моделей на различных датасетах.
...
usage: train.py [-h] [--n_epochs N_EPOCHS] [--lr LR] [--num_workers NUM_WORKERS] [--batch_size BATCH_SIZE] [--device DEVICE] [--unaligned UNALIGNED] [--img_size IMG_SIZE]
                [--lambda_value LAMBDA_VALUE] [--datasets_dir DATASETS_DIR] [--dataset_name DATASET_NAME] [--train_hist_dir TRAIN_HIST_DIR] [--n_imgs_save N_IMGS_SAVE]
...
options:
  ...
  --device DEVICE       Device on which calculations will be performed
  --unaligned UNALIGNED
                        Attachment of indexes of images of different types to each other
  ...
  --train_hist_dir TRAIN_HIST_DIR
                        Directory with training history for different datasets
  --n_imgs_save N_IMGS_SAVE
                        The number of test images to save at the epoch with results. Default = 0
  --weights_dir WEIGHTS_DIR
                        Directory from where you can download or save the weights of various datasets
```

<a name="eval_interface"><h3>4.2. Запуск генерации изображений</h3></a> 

Чтобы запустить генерацию изображений, необходимо указать директорию, где находятся изображения, необходимые для преобразования в изображения другого типа.

Изображения для преобразования должны храниться в следующем виде:

```
├── imgs_to_eval            # Папка с изображениями для преобразовния
...
Если папки с изображениями непустые, то создадутся папки с результатами (для A_type - результаты преобразования A->B; для B_type - результаты преоразования B->A).

Папки с изображениями могут быть пустыми. Тогда в итоге создадутся просто пустые папки с результатами.

Также необходимо указать путь к папе, где находятся веса модели. Если такая папка не была найдена, то выполнится преобразование для модели CycleGAN без предобученных весов.
...
* Создан интерфейс, с помощью которого можно начинать новое обучение модели или продолжать её обучение. Также он позволяет выполнять генерацию изображений одного вида в изображения другого вида.